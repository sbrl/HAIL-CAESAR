	...
	if (int(peak_indices_temp.size()) > 1)
	{
		...
		for (int i = 0; i < int(peak_indices_sorted.size()); i++)
		{
			if (check_peaks[peak_indices_sorted[i]] == 1)
			{
				...
				if (max_dist >= int(check_peaks.size()))
				{
					max_dist = int(check_peaks.size()) - 1;
				}
...
float get_percentile(vector <float>& data, float percentile)
{
	...
	if (k >= N - 1)
	{
		percentile_value = data[N - 1];
	}
	else if (k < 0)
	{
		percentile_value = data[0];
	}
	else
	{
		percentile_value = data[k] + d * (data[k + 1] - data[k]);
	}
...
{
	for (int i = 0; i < int(DSnodes.size()); ++i)
	{
		delete DSnodes[i];
	}
	DSnodes.clear();
...
{
	if (elements + 1 > int(DSnodes.size()))
	{
		DSnodes.push_back(new DSnode);
	}
...
void DisjointSet::Union(DSnode *node1, DSnode *node2)
{
	if (node1 == node2)
	{
		return;                          // same node - no action required
	}
	DSnode *parent1 = Find(node1);
	DSnode *parent2 = Find(node2);
	if (parent1 == parent2)
	{
		return;                              // same set - no action required
	}
...
int DisjointSet::Union_return_label(DSnode *node1, DSnode *node2)
{
	if (node1 == node2)
	{
		return(node1->parent->i_node);                         // same node - no action required
	}
	DSnode *parent1 = Find(node1);
	DSnode *parent2 = Find(node2);
	if (parent1 == parent2)
	{
		return(parent1->i_node);                             // same set - no action required
	}
	if (parent1->rank < parent2->rank)
	{
		parent1->parent = parent2;

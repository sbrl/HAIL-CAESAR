...
{
	//cerr << "You need to initialize with a filename!" << endl;
	//exit(EXIT_FAILURE);
}
							...
							float cellsize, int ndv, Array2D <int> data)
{
	...
	{
		cerr << "LSDIndexRaster create::dimension of data is not the same as stated in NRows!" << endl;
		exit(EXIT_FAILURE);
	}
														   ...
														   vector <int>& row_nodes, vector <int>& col_nodes)
{
	...
	cerr << "Initial sweep. " << endl;
	for (int col = 0; col < NCols; col++)
	{
		if (col % 250 == 0)
		{
			cerr << "Column " << col << " of " << NCols << endl;
		}
	...
	cerr << "Running rows" << endl;
	for (int row = 0; row < NRows; row++)
	{
		if (row % 250 == 0)
		{
			cerr << "Row " << row << " of " << NRows << endl;
		}
	...
	for (int sweep = 0; sweep < NSweeps; sweep++)
	{
		...
		for (int row = 0; row < NRows; row++)
		{
			for (int col = 0; col < NCols; col++)
			{
				...
				{
					//cerr << "Getting this one, visted is: " << Visited[row][col] << endl;
...
void LSDIndexRaster::release_random_bot(Array2D <int>& Visited, int startrow, int startcol, int NSteps)
{
	...
	if (RasterData[startrow][startcol] == NoDataValue)
	{
		...
		{
			//cerr << "Curr row: " << curr_row << " and col: " << curr_col << endl;
...

{
	...
	{
		cerr << "Array dimensions do not match template LSDIndexRaster object" << endl;
		exit(EXIT_FAILURE);
	}
}
...
{
	cerr << "\t\t Connected Components; first pass" << endl;
		...
		for (int j = 0; j < NCols; ++j)
		{
			...
			if (RasterData[i][j] == 1)
			{
				...
				else
				{
					...
					if (i == 0)
					{
						if (LabelledComponents[i][j - 1] != NoDataValue)
						{
							LabelledComponents[i][j] = LabelledComponents[i][j - 1];
						}
						else
						{
							LabelledComponents[i][j] = next_label;
					...
					else if (j == 0)
					{
						...
						{
							neighbourhood_labels.push_back(above);
						}
						if (above_right != NoDataValue)
						{
							neighbourhood_labels.push_back(above_right);
						}
					...
					else if (j == NCols - 1)
					{
						...
						if (left != NoDataValue)
						{
							neighbourhood_labels.push_back(left);
						}
...
LSDIndexRaster LSDIndexRaster::thin_to_skeleton()
{
	...
	while (finish_flag == 0)
	{
		cerr << flush << "Thinning - iteration number " << count << "; ";
		++count;
		int removed = 0;
		finish_flag = 1;
		...
		cerr << "removed " << removed << " pixels; " << total_removed << "; removed in total     \r";
...
void LSDIndexRaster::remove_downstream_endpoints(LSDIndexRaster CC, LSDRaster Topo)
{
	...
	for (int i = 0; i < NRows; ++i)
	{
		for (int j = 0; j < NCols; ++j)
		{
			if (RasterData[i][j] != NoDataValue && CC.get_data_element(i, j) > max_segment_ID)
			{
				max_segment_ID = CC.get_data_element(i, j);
			}
		}
	}
...
LSDIndexRaster LSDIndexRaster::filter_by_connected_components(int connected_components_threshold)
{
	...
	for (int i = 0; i < NRows; ++i)
	{
		for (int j = 0; j < NCols; ++j)
		{
			...
			{
				IDs.push_back(ConnectedComponentsRaster.get_data_element(i, j));
			}
		}
	}
	vector <size_t> index_map;
		...
		for (int col = 0; col < NCols; col++)
		{
			...
			if (RasterArray[row][col] == 0)
			{
				...
				for (int i = 1; i <= pixel_radius; i++)
				{
					...
					if (RasterArray[min_row][min_col] == 1)
					{
						counts.at(0) = 1;
					}
					if (RasterArray[row][min_col] == 1)
					{
						counts.at(1) = 1;
					}
					if (RasterArray[max_row][min_col] == 1)
					{
						counts.at(2) = 1;
					}
					if (RasterArray[min_row][col] == 1)
					{
						counts.at(3) = 1;
					}
					if (RasterArray[min_row][max_col] == 1)
					{
						counts.at(4) = 1;
					}
					if (RasterArray[row][max_col] == 1)
					{
						counts.at(5) = 1;
					}
					if (RasterArray[max_row][max_col] == 1)
					{
						counts.at(6) = 1;
					}
					if (RasterArray[max_row][col] == 1)
					{
						counts.at(7) = 1;
					}
			...
			if (RasterArray[row][col] == 2)
			{
				FilledRaster[row][col] = 2;
			}
		}
	}
...
LSDIndexRaster LSDIndexRaster::remove_holes_in_patches_connected_components(int window_radius)
{
	...
	if (window_radius < DataResolution)
	{
		pixel_radius = int(DataResolution);
	}
	cerr << "Pixel radius: " << pixel_radius << endl;
		...
		for (int col = 0; col < NCols; col++)
		{
			...
			if (RasterArray[row][col] == 0)
			{
				...
				if (count == 4)
				{
					//fill in pixel
					FilledArray[row][col] = 1;
					//cerr << "Filled in pixel, woohoo" << endl;
				}
				count = 0;
			}
		}
...
vector <float> LSDIndexRaster::AnalysisOfQuality(LSDIndexRaster& ActualRaster)
{
	...
	//cerr << "Got the total TPs and FPs" << endl;
	cerr << "SumTP = " << SumTP << " SumFP = " << SumFP << " SumTN = " << SumTN << " SUM FN = " << SumFN << endl;
...
float LSDIndexRaster::GetAreaDifference(LSDIndexRaster& ActualRaster)
{
	...
	for (int i = 0; i < NRows; i++)
	{
		for (int j = 0; j < NCols; j++)
		{
			if (ActualRaster_array[i][j] == 1)
			{
				++NActualPixels;
			}
			if (PredictedRaster_array[i][j] == 1)
			{
				++NPredictedPixels;
			}
		}
	}
	cerr << "N predicted pixels: " << NPredictedPixels << " N actual pixels: " << NActualPixels << endl;
